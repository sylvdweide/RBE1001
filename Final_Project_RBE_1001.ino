...
const int left = 28;      //set pins to match actual pin values
const int right = 29;     //set pins to match actual pin values
const int IR_PIN = 2;     //set pins to match actual pin values
...
int IRSteady()      //averages 10 IR readings to make the values more regular
{
  int sum = 0;
  for(int n = 0; n < 10; n++)
  {
    sum = sum + analogRead(IR_PIN);
  }
  return sum/10;
}

bool sideEdgeLocator()
{
  if(IRSteady >= 600)
  {
    return true;
